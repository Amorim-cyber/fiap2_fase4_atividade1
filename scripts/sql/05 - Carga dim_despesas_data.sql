BEGIN    
    DECLARE v_dataInicial DATE;
     ...
     v_skData INTEGER;
    BEGIN
    -- Digite o período que você deseja carregar na dimensão data
    v_dataInicial := '01/01/2000';
    v_dataFinal := '31/12/2030';
    ...
    ELSE 5
    END;
    IF (v_data >= (TO_DATE('23/09/' || TO_CHAR(v_ano)))) AND (v_data <=
    (TO_DATE('20/12/' || TO_CHAR(v_ano)))) THEN
    v_estacaoAno := 'primavera';
    ELSIF (v_data >= (TO_DATE('21/03/' || TO_CHAR(v_ano)))) AND (v_data <=
    (TO_DATE('20/06/' || TO_CHAR(v_ano)))) THEN
    v_estacaoAno := 'outono';
    ELSIF (v_data >= (TO_DATE('21/06/' || TO_CHAR(v_ano)))) AND (v_data <=
    (TO_DATE('22/09/' || TO_CHAR(v_ano)))) THEN
    v_estacaoAno := 'inverno';
    ELSE -- v_data between 21/12 e 20/03
    v_estacaoAno := 'verão';
    END IF;
    INSERT INTO dim_despesas_data
    SELECT
    ...
    v_estacaoAno ,
    lower(v_nomeDiaSemana || ', ' || TO_CHAR(v_dia) || ' de ' ||
    v_nomeMes || ' de ' || TO_CHAR(v_ano)), -- Data
    v_dataInicial FROM DUAL;
    commit;
